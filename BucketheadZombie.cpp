...
BucketheadZombie::BucketheadZombie(int x, int y)
{
    ...
    this->exists = true;
    this->speed = 2;
    this->zombieTexture.loadFromFile("assets\\Spritesheets\\bucHeadZombWalk.png");
...
void BucketheadZombie::drawZombie(RenderWindow& window)
{
    if (!zombieEats)
        animate(0.09, 1575, 105, 144);
    else
        animate(0.09, 1155, 105, 144);
...
void BucketheadZombie::eat()
{
    if (zombieEats)
    {
        this->zombieEatTexture.loadFromFile("assets\\Spritesheets\\bucHeadZombEat.png");
